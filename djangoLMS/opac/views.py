...
def home(request):
    ...
    if(quotes_count>0):
      ...
      quotation = Quotations.objects.raw(query)
        ...
        quotation = [{'source':'గాంధీజీ','text':'మంచి పుస్తకం దగ్గర ఉంటే మనకు మంచి మిత్రుడు వెంటలేని లోటు కనిపించదు.'},]
    context = {
...
def catalog_search(request):
    ...
    return render(request,'opac/search/catalog_search.html', {
        'q': q,
        'error': error,
...
class AjaxableVisitorsResponseMixin:
    def get(self, request):
       ...
       return render(request,'opac/search/entryexitlogs_search.html', {
          'q': q,
          'error': error,
          'search_results': query,
    ...
    def form_valid(self, form):
        ...
        query = EntryExitLogs.objects.filter(timeofentry__contains=today).order_by('-id')
        #return render_to_response('opac/search/entryexitlogs_search.html', {
        return render(self.request, 'opac/search/entryexitlogs_search.html', {
          'q': '',
          'error': '',
          'search_results': query,
          'introspections': json.dumps(EntryExitLogsQLSchema(query.model).as_dict()),
...
def entryexitlog_search(request):
    ...
    return render(request,'opac/search/entryexitlogs_searchql.html', {
        'q': q,
        'error': error,
        'search_results': query,
        'introspections': json.dumps(EntryExitLogsQLSchema(query.model).as_dict()),
